...
anaExpr (Out.Or e1 e2 p) = do
    ...
    case et2 of
        ...
        Context.ExprTable Context.Boolean Context.Dynamic n -> do
            let st = modifyTable popTable st
            put ( modifyTable (pushTable (Context.ExprTable Context.Boolean Context.Dynamic n)) st )
            return ()
        Context.ExprTable Context.Boolean c n -> do
            case et1 of
                ...
                Context.ExprTable Context.Boolean c (Context.CBoolean n1) -> do
                    let st = modifyTable popTable st
                    put ( modifyTable (pushTable (Context.ExprTable Context.Boolean c (modifyBoolValCalc (n1||) n))) st )
                    return ()
    ...
    case et2 of
        Context.ExprTable Context.Boolean Context.Dynamic n2 -> do 
            case et1 of
                Context.ExprTable Context.Boolean _ _ -> do
                    let st = modifyTable popTable st
                    put ( modifyTable (pushTable (Context.ExprTable Context.Boolean Context.Dynamic n2)) st )
                    return ()
                _ -> do
                    throw $ Context.ContextError ("Cerca de la siguiente posicion" 
                                                    ++ (Out.printPos p)
        ...
        Context.ExprTable Context.Boolean c2 (CBoolean n2) -> do 
            case et1 of
                Context.ExprTable Context.Boolean Context.Dynamic n1 -> do
                    let st = modifyTable popTable st
                    put ( modifyTable (pushTable (Context.ExprTable Context.Boolean Context.Dynamic n1)) st )
                    return ()
                Context.ExprTable Context.Boolean c1 (CBoolean n1) -> do
                    let st = modifyTable popTable st
                    put ( modifyTable (pushTable (Context.ExprTable Context.Boolean c1 (comparisonFunBool (==) n1 n2))) st )
                    return ()
                _ -> do
                    throw $ Context.ContextError ("Cerca de la siguiente posicion" 
                                                    ++ (Out.printPos p)
        ...
        Context.ExprTable Context.Number Context.Dynamic n2 -> do 
            case et1 of
                Context.ExprTable Context.Number _ _ -> do
                    let st = modifyTable popTable st
                    put ( modifyTable (pushTable (Context.ExprTable Context.Number Context.Dynamic n2)) st )
                    return ()
                _ -> do
                    throw $ Context.ContextError ("Cerca de la siguiente posicion" 
                                                    ++ (Out.printPos p)
        ...
        Context.ExprTable Context.Number c2 (CNumber n2) -> do 
            case et1 of
                Context.ExprTable Context.Number Context.Dynamic n1 -> do
                    let st = modifyTable popTable st
                    put ( modifyTable (pushTable (Context.ExprTable Context.Number Context.Dynamic n1)) st )
                    return ()
                Context.ExprTable Context.Number c1 (CNumber n1) -> do
                    let st = modifyTable popTable st
                    put ( modifyTable (pushTable (Context.ExprTable Context.Number c1 (comparisonFunNum (==) n1 n2))) st )
                    return ()
                _ -> do
                    throw $ Context.ContextError ("Cerca de la siguiente posicion" 
                                                    ++ (Out.printPos p)
...
anaExpr (Out.Less e1 e2 p) = do
    ...
    case et2 of
        ...
        Context.ExprTable Context.Number Context.Dynamic n -> do
            let st = modifyTable popTable st
            put ( modifyTable (pushTable (Context.ExprTable Context.Number Context.Dynamic n)) st )
            return ()
        Context.ExprTable Context.Number c n@(Context.CNumber n2) -> do
            case et1 of
                ...
                Context.ExprTable Context.Number c (Context.CNumber n1) -> do
                    let st = modifyTable popTable st
                    put ( modifyTable (pushTable (Context.ExprTable Context.Number c (comparisonFunNum (<) n1 n2))) st )
                    return ()
    ...
    case et2 of
        ...
        Context.ExprTable Context.Number c n -> do
            case et1 of
                ...
                Context.ExprTable Context.Number c (Context.CNumber n1) -> do
                    let st = modifyTable popTable st
                    put ( modifyTable (pushTable (Context.ExprTable Context.Number c (modifyDoubleValCalc (n1+) n))) st )
                    return ()
    ...
    case et2 of
        ...
        Context.ExprTable Context.Number c n -> do
            case et1 of
                ...
                Context.ExprTable Context.Number c (Context.CNumber n1) -> do
                    let st = modifyTable popTable st
                    put ( modifyTable (pushTable (Context.ExprTable Context.Number c (modifyDoubleValCalc (\x -> n1 / x) n))) st )
                    return ()
    ...
    case et2 of
        ...
        Context.ExprTable Context.Number c n -> do
            case et1 of
                ...
                Context.ExprTable Context.Number c (Context.CNumber n1) -> do
                    let st = modifyTable popTable st
                    put ( modifyTable (pushTable (Context.ExprTable Context.Number c (modifyDoubleValCalc (\x -> modex n1 x) n))) st )
                    return ()
    ...
    case et2 of
        ...
        Context.ExprTable Context.Number c n -> do
            case et1 of
                ...
                Context.ExprTable Context.Number c (Context.CNumber n1) -> do
                    let st = modifyTable popTable st
                    put ( modifyTable (pushTable (Context.ExprTable Context.Number c (modifyDoubleValCalc (applyIntegerFun div n1) n))) st )
                    return ()
    ...
    case et2 of
        ...
        Context.ExprTable Context.Number c n -> do
            case et1 of
                ...
                Context.ExprTable Context.Number c (Context.CNumber n1) -> do
                    let st = modifyTable popTable st
                    put ( modifyTable (pushTable (Context.ExprTable Context.Number c (modifyDoubleValCalc (applyIntegerFun mod n1) n))) st )
                    return ()
...
anaExpr (Out.Not e p) = do
    ...
    case topTable $ tablas st of
        ...
        Context.ExprTable Context.Boolean c n -> do
            let st = modifyTable popTable st
            put ( modifyTable (pushTable (Context.ExprTable Context.Boolean c (modifyBoolValCalc (\x -> not x) n))) st )
            return ()
        _ -> do
            error "Error interno, algo salio mal y no esta la tabla de la expresion"
...
anaExpr (Out.Uminus e p) = do
    ...
    case topTable $ tablas st of
        ...
        Context.ExprTable Context.Number c n -> do
            let st = modifyTable popTable st
            put ( modifyTable (pushTable (Context.ExprTable Context.Number c (modifyDoubleValCalc (\x -> -x) n))) st )
            return ()
        _ -> do
            error "Error interno, algo salio mal y no esta la tabla de la expresion"
    ...
    case topTable $ tablas st of
        FuncionTable r -> do 
            case r of
                ...
                _ -> do
                    let st = modifyTable popTable st
                    put ( modifyTable (pushTable (Context.ExprTable r Context.Dynamic Context.Nein)) st )
                    return ()
        _ -> do
